...
- **bin/** — scripts on `PATH`: git helpers (`git-undo`, `git-amend`,
  `git-delete-merged`, `git-force-push`), `wtclean` (worktree/branch cleanup, see
...
- **worktrunk/** — worktrunk config; drives the `wtpr`/`wtprx`/`wtclean` worktree +
  ...
  stays with `wtclean`/`wt remove`. The `wt` shell integration is an
  ...
  `bin/wtclean` (bash) sweeps merged worktrees and branches: it asks `wt list
  --format json --branches` for rows whose state is `integrated` or `empty` and feeds
  them to `wt remove`, after a `git fetch --prune`. Given a repo it sweeps that repo;
  given a directory that isn't one it sweeps each immediate child that is (siblings
  sharing a `.git` dir are swept once). It replaced the earlier `wtsweep` script and
  the fish `wtclean` function, which were the same sweep at two scopes. `-n` dry-runs.
  The filter must exclude `repo.default_branch` **by name**, not just rows where
  ...
  a filter keyed only on `worktree.main` will happily delete it. The jq is written
  against schema 2, so the list call pins it with `--config-set list.json-schema=2`
  rather than trusting `[list] json-schema` in `config.toml`.
